...
- `/mn-review` - Performs thorough code reviews as a project maintainer. Accepts a PR number or URL, or reviews local worktree changes when no arguments provided. Covers code quality, security, performance, testing, and documentation. Automatically detects languages/frameworks from changed files and applies language-specific review guidelines (React, TypeScript, JavaScript, Go, Java, Bash, Makefile, CSS).
...
- `/mn-session-retrospective` - Reviews the just-completed session for friction (permission prompts on safe read-only ops, sandbox blocks, chained/piped commands that defeated the allowlist, rabbit holes, wrong/missing guidance) and proposes concrete edits to `settings.json`, the global `CLAUDE.md`, and the repo's skills so the next session runs more autonomously. Dynamically resolves the repo via `$MN_PROJECTS/manusa/ai`; every file edit is gated behind explicit confirmation.
